<?php
    class ProductRepository extends BaseRepository {
        protected function fetchAll($condition = null, $sort = null, $limit = null){
            ...
            if($result->num_rows > 0){
                while($row = $result->fetch_assoc()){
                    $product = new Product(
                        $row["id"], $row["discount_id"], $row["name"], $row["price"],
                        $row["featured_image"], $row["inventory_qty"], $row["category_id"],
                        $row["brand_id"], $row["created_date"], $row["description"],
                        $row["enter_price"], $row["deleted"], $row["updated_at"], $row["featured"]
                    );
                    $products[] = $product;
                }
            }
        ...
        function getBy($array_conds = array(), $array_sorts = array(), $page = null, $qty_per_page = null){
            ...
            $temp = array();
            // column là tên cột trong cơ sở dữ liệu và cond là một mảng con chứa thông tin về loại điều kiện và giá trị tương ứng.
            foreach($array_conds as $column => $cond){
                // trích xuất loại điều kiện và giá trị tương ứng từ mỗi phần tử trong mảng.
                $type = $cond["type"];
                $val = $cond["val"];
            ...
            // Mảng có ít nhất 1 phần tử
            if(count($array_conds)){
                $condition = implode(" AND ", $temp);
            }

            $temp = array();
            // key là tên cột cần sắp xếp và sort là hướng sắp xếp (ASC hoặc DESC).
        ...
        function save($data){
            ...
            $sql = "INSERT INTO product (name, price, featured_image, inventory_qty, category_id, brand_id, 
                    create_date, description, enter_price, deleted, updated_at, featured)
                    VALUES ('$name', $price, '$featured_image', $inventory_qty, $category_id, $brand_id, 
                    '$created_date', '$description', $enter_price, $deleted, '$updated_at', '$featured')";
